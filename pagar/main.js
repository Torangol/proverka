...
// let ila = document.querySelector('#ILYA');
// let items = document.querySelectorAll('#ILYA li');
...
// const userNames = ["Ilya", "Lev", "Heralt"];
...
//     const lastA = splitA[splitA.lenght - 1];
//     const lastB = splitB[splitB.lenght - 1];
...
const btn: Element = document.querySelector(selectors '.search');
const input: Element = document.querySelector(selectors '.input');
const list: Element = document.querySelector(selectors '.list');
let users;

const getUsers = async () => {
    const response: response = await fetch('jsonplaceholder.typicode.com/users');
...
const setAllUsers = (users) => {
    users.innerHTML = '';
    users.forEach(user => {
        list.innerHTML += getUserHTML(user);
    });
};
...
btn.addEventListener('click', () => {
    const filteredUsers = filterUsers(users, input.value);
